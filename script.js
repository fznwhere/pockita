...
function initMonthPicker() {
    ...
    const dayPicker = document.getElementById('filter-hari');
...
function changeGlobalMonth() {
    ...
    if(document.getElementById('page-analysis').style.display === 'block') renderCharts();

}



// FORMAT UANG LIVE SAAT DIKETIK (Global Class .format-uang)
...
function formatK(angka) {
    ...
    if (angka >= 1000) {
        ...
        if (angka % 1000 === 0) return (angka / 1000) + 'K';

        return (angka / 1000).toFixed(1).replace('.0', '') + 'K';
...
function populateSelectMethods() {
    ...
    const elementsToUpdate = ['in-metode', 'out-metode', 'recur-metode'];
...
function saveNewMethod() {
    ...
    if (fileInput.files && fileInput.files[0]) {
        ...
        reader.onload = function(e) {
            ...
            pockitaMethods[name] = e.target.result; 

            finalizeSaveMethod(nameInput, fileInput);
...
function renderApp() {
    ...
    if (currentHistoryMode === 'bulan') {
        ...
        displayTransactions = transactions.filter(tx => tx.date.startsWith(activeMonth));
    ...
    if (currentFilter !== 'Semua') {
        ...
        displayTransactions = displayTransactions.filter(tx => tx.method === currentFilter);
        ...
        for (const method in groupedTx) {
            ...
            sortedDates.forEach(dateStr => {
                ...
                txsByDate[dateStr].sort((a, b) => {
                    ...
                    let isPiutang = tx.desc.startsWith('Memberi Piutang: ') || tx.desc.startsWith('Terima Piutang: ');

                    

                    let cleanDesc = tx.desc.replace('Auto: ', '').replace('Target: ', '').replace('Memberi Piutang: ', '').replace('Terima Piutang: ', '');
...
function editTransaksi(id) {
    ...
    if(tx.type === 'income') {
        ...
        let cleanText = tx.desc.replace('Auto: ', '').replace('Target: ', '').replace('Memberi Piutang: ', '').replace('Terima Piutang: ', '');
...
document.getElementById('form-piutang').addEventListener('submit', function(e){
    ...
    savingsGoals.push({ id: piutangId, type: 'piutang', name, target, note, method, category });
...
function tambahTabunganGoal(id) {
    ...
    // PENANGANAN BUG: PERHITUNGAN COLLECTED PIUTANG JIKA TIPE ADALAH INCOME (CICILAN MASUK)

    if (goal.type === 'piutang') {

        collectedAmount = transactions.filter(tx => tx.goalId === goal.id && tx.type === 'income').reduce((sum, tx) => sum + tx.amount, 0);
    ...
    // LOGIKA PERUBAHAN TIPE TRANSAKSI UNTUK PIUTANG (INCOME) VS GOAL (EXPENSE)

    const txType = goal.type === 'piutang' ? 'income' : 'expense';

    const txDesc = goal.type === 'piutang' ? `Terima Piutang: ${goal.name}` : `Target: ${goal.name}`;
...
function hapusPlan(type, id) {
    ...
    if(confirm("Hapus item rencana ini? (Menghapus Target tidak mengembalikan saldo transaksi yang sudah ada)")){
...
function renderPlans() {
    ...
    if(recurringTemplates.length === 0){
        ...
        recurringTemplates.forEach(t => {
            ...
            let catColorClass = t.category.toLowerCase() === 'kebutuhan' ? 'pill-kebutuhan' : 'pill-keinginan';
                ...
                <li class="plan-item">
                    ...
                    <span><b>${t.name}</b> Rp ${formatRupiah(t.amount)}<br>
                        ...
                        <div class="pill-container" style="margin-top: 4px;">
                            ...
                            <span class="pill ${catColorClass}">${t.category.toUpperCase()}</span>

                            <span class="pill pill-method">${t.method}</span>
    ...
    // PENGGABUNGAN DATA GOAL DAN PIUTANG UNTUK DIRENDER BERSAMAAN

    if(savingsGoals.length === 0){

        containerGoals.innerHTML = `<div class="empty-state"><img src="pochita-sleep.png" class="empty-state-img empty-neon-orange" alt="Kosong">Belum ada target menabung atau piutang.</div>`;

    } else {

        savingsGoals.sort((a, b) => b.id - a.id).forEach(g => {

            let collectedAmount = 0;

            let barColor = 'var(--primary-orange)';

            let catDisplay = g.category ? g.category.toUpperCase() : 'TABUNGAN';

            let noteUI = '';



            // LOGIKA KHUSUS PIUTANG (MENGHITUNG INCOME, WARNA BIRU)

            if (g.type === 'piutang') {

                collectedAmount = transactions.filter(tx => tx.goalId === g.id && tx.type === 'income').reduce((sum, tx) => sum + tx.amount, 0);

                barColor = '#3498DB'; // Biru Piutang

                catDisplay = `${g.category.toUpperCase()} | Diambil dari ${g.method}`;

                if (g.note) noteUI = `<div class="piutang-note">📝 Catatan: ${g.note}
